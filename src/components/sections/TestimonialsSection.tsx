import { Star, CheckCircle } from 'lucide-react'
import { cn } from '@/lib/utils'
import { BUSINESS } from '@/lib/constants'
import type { GoogleReview } from '@/lib/reviews'

// ─── Google "G" logo (inline SVG — no external request) ─────────────────────

function GoogleLogo({ size = 16 }: { size?: number }) {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      aria-label="Google"
      role="img"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
        fill="#4285F4"
      />
      <path
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
        fill="#34A853"
      />
      <path
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z"
        fill="#FBBC05"
      />
      <path
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
        fill="#EA4335"
      />
    </svg>
  )
}

// ─── Reviewer avatar — colored circle with first initial ────────────────────

const AVATAR_COLORS = [
  'bg-blue-500',
  'bg-emerald-500',
  'bg-violet-500',
  'bg-rose-500',
  'bg-amber-500',
  'bg-cyan-500',
  'bg-indigo-500',
  'bg-teal-500',
]

function ReviewerAvatar({ name }: { name: string }) {
  const initial = name.trim().charAt(0).toUpperCase()
  // Deterministic color from name so it never changes between renders
  const colorClass =
    AVATAR_COLORS[
      name.split('').reduce((acc, c) => acc + c.charCodeAt(0), 0) %
        AVATAR_COLORS.length
    ]
  return (
    <div
      className={cn(
        'flex items-center justify-center w-8 h-8 rounded-full text-white text-sm font-bold shrink-0',
        colorClass,
      )}
      aria-hidden="true"
    >
      {initial}
    </div>
  )
}

// ─── Individual review card ──────────────────────────────────────────────────

function ReviewCard({ review }: { review: GoogleReview }) {
  return (
    <blockquote className="flex flex-col bg-white rounded-xl p-6 border border-brand-border shadow-sm hover:shadow-md transition-shadow duration-200">
      {/* Header: Google badge + stars */}
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-1.5">
          <GoogleLogo size={15} />
          <span className="text-xs font-medium text-brand-muted">Google Review</span>
        </div>
        <div className="flex items-center gap-0.5" aria-label="5 out of 5 stars">
          {Array.from({ length: 5 }).map((_, i) => (
            <Star key={i} size={13} className="text-brand-amber fill-brand-amber" aria-hidden="true" />
          ))}
        </div>
      </div>

      {/* Review text */}
      <p className="text-brand-text text-sm leading-relaxed italic flex-1 mb-5">
        &ldquo;{review.text}&rdquo;
      </p>

      {/* Footer: avatar + name + time + verified badge */}
      <footer>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <ReviewerAvatar name={review.authorName} />
            <cite className="font-semibold text-brand-charcoal not-italic text-sm">
              {review.authorName}
            </cite>
          </div>
          {review.relativeTime && (
            <span className="text-brand-muted text-xs shrink-0 ml-2">{review.relativeTime}</span>
          )}
        </div>
        <div className="flex items-center gap-1 mt-2 ml-[2.625rem]">
          <CheckCircle size={11} className="text-emerald-500 shrink-0" aria-hidden="true" />
          <span className="text-xs text-brand-muted">Verified Google Review</span>
        </div>
      </footer>
    </blockquote>
  )
}

// ─── Section props ───────────────────────────────────────────────────────────

interface TestimonialsSectionProps {
  className?: string
  title?: string
  reviews?: GoogleReview[]
  maxItems?: number
}

export function TestimonialsSection({
  className,
  title = 'What Brooklyn Customers Say',
  reviews = [],
  maxItems = 6,
}: TestimonialsSectionProps) {
  const displayed = reviews.slice(0, maxItems)
  if (displayed.length === 0) return null

  return (
    <section className={cn('py-14 md:py-20 bg-brand-bg-alt', className)}>
      <div className="container mx-auto px-4">
        {/* Section heading */}
        <div className="text-center mb-4">
          <div className="flex items-center justify-center gap-1 mb-3">
            {Array.from({ length: 5 }).map((_, i) => (
              <Star key={i} size={22} className="text-brand-amber fill-brand-amber" aria-hidden="true" />
            ))}
          </div>
          <h2 className="text-3xl md:text-4xl font-bold text-brand-charcoal mb-2">{title}</h2>
          <p className="text-brand-muted">
            {BUSINESS.rating}/5 stars &mdash; {BUSINESS.reviewCount}+ verified Google reviews
          </p>
        </div>

        {/* Cards grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mt-10">
          {displayed.map((review) => (
            <ReviewCard key={review.id} review={review} />
          ))}
        </div>

        {/* CTA */}
        <div className="text-center mt-8">
          <a
            href={BUSINESS.reviewsUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 text-sm text-brand-navy hover:underline font-medium"
          >
            <GoogleLogo size={14} />
            Read all reviews on Google
          </a>
        </div>
      </div>
    </section>
  )
}
